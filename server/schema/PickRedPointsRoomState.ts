...
export class PublicPickRedPlayer extends Schema {
  id = ""; nickname = ""; seat = 0; type = "human"; status = "playing"; connected = true; ready = false; cardsRemaining = 0; capturedCount = 0; score = 0; scoreAdjustment = 0; botDifficulty = ""; host = false;
}
defineTypes(PublicPickRedPlayer, { id: "string", nickname: "string", seat: "number", type: "string", status: "string", connected: "boolean", ready: "boolean", cardsRemaining: "number", capturedCount: "number", score: "number", scoreAdjustment: "number", botDifficulty: "string", host: "boolean" });
...
export class PickRedPointsRoomStateSchema extends Schema {
  roomCode = ""; phase = "waiting"; maxPlayers = 4; round = 1; currentPlayerId = ""; turnNumber = 0; turnDeadline = 0; targetDeadline = 0; legalTargetIds = ""; drawPileCount = 0; lastResult = ""; winners = ""; blackHandEligiblePlayerIds = ""; blackHandPendingPlayerIds = ""; revealedBlackHandPlayerId = ""; pendingCard: PublicPickRedCard | null = null; players = new ArraySchema<PublicPickRedPlayer>(); tableCards = new ArraySchema<PublicPickRedCard>(); revealedBlackHandCards = new ArraySchema<PublicPickRedCard>();
}
...
export function syncPickRedPublicState(schema: PickRedPointsRoomStateSchema, state: PickRedPointsState | null, lobbyPlayers: LobbyPickRedPlayer[], turnDeadline = 0) {
  ...
  (state ? state.players.map((player) => ({ ...player, ready: true, cardsRemaining: state.hands[player.id]?.length ?? 0, capturedCount: state.capturedCards[player.id]?.length ?? 0, score: state.scores[player.id] ?? 0, scoreAdjustment: scoreAdjustments[player.id] ?? 0, connected: player.type === "bot" || Boolean(lobbyPlayers.find((item) => item.id === player.id)?.connected), host: Boolean(lobbyPlayers.find((item) => item.id === player.id)?.host) })) : lobbyPlayers).sort((left, right) => left.seat - right.seat).forEach((player) => {
    const target = new PublicPickRedPlayer(); Object.assign(target, { id: player.id, nickname: player.nickname, seat: player.seat, type: player.type, status: "status" in player ? player.status : "waiting", connected: player.connected, ready: player.type === "bot" || player.ready, cardsRemaining: "cardsRemaining" in player ? player.cardsRemaining : 0, capturedCount: "capturedCount" in player ? player.capturedCount : 0, score: "score" in player ? player.score : 0, scoreAdjustment: "scoreAdjustment" in player ? player.scoreAdjustment : 0, botDifficulty: player.botDifficulty ?? "", host: player.host }); schema.players.push(target);
  });
