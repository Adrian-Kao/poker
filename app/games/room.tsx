  ...
  return (
    <article className={`bluff-opponent-seat room-opponent-seat ${position} ${active ? "active" : ""} ${player.status === "finished" ? "finished" : ""}`}>
      <div className="bluff-player-badge">
        ...
        <div><strong>{player.nickname}</strong><span>{count} 張牌</span></div>
...
export function RoomSelfBadge({ nickname, active = false, count }: { nickname: string; active?: boolean; count?: number }) {
  return (
    <div className={`bluff-self-badge room-self-badge ${active ? "active" : ""}`}>
      ...
      <div><span>你的手牌</span><strong>{nickname}</strong>{typeof count === "number" ? <em>{count} 張牌</em> : null}</div>
    </div>
  );
}
    ...
    <main className="heart-auto-shell ninety-online-shell">
      ...
      <section className="heart-waiting-room ninety-waiting-room unified-waiting-room">
        ...
        <div className="heart-lobby-actions">
          ...
          {isHost && allowBots && onAddBot ? <button type="button" className="ready-button bot-button" onClick={onAddBot} disabled={!canUseRoom || players.length >= maxPlayers}><Bot size={22} />加電腦補位</button> : null}
